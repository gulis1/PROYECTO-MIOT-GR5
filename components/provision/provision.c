                                          ...
                                          uint8_t **outbuf, ssize_t *outlen, void *priv_data)
{
    ...
    int urlSize = 0;
    char *input; 
    input = malloc(sizeof(char)*inlen);
    if (inbuf) {
        ESP_LOGI(TAG, "Received data: %.*s", inlen, (char *)inbuf);
        input = strdup((char *)inbuf);
        int dataPos = 0;
        while(dataPos!=3)
        {
            if(dataPos == 0)
            ...
            {
                if(input[pisoSize+aulaSize+urlSize] == ';')
                {
                    dataPos++;
        ...
        ESP_LOGI(TAG, "piso :%s",PROVISION_INFO.data_piso);
        ESP_LOGI(TAG, "aula :%s",PROVISION_INFO.data_aula);
        ESP_LOGI(TAG, "url :%s",PROVISION_INFO.data_url);
    }
