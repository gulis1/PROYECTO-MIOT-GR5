...
{
    uint8_t direccion[MAC_LEN];
    int intensidad;
};

struct direccion_mac lista_mac[MAX_MACS];

//comparacion de direcciones mac
int comparar_mac(struct direccion_mac *mac1, struct direccion_mac *mac2){
    return memcmp(mac1->direccion, mac2->direccion, MAC_LEN)==0;
}
