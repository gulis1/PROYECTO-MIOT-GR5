...
static const char *TAG_2 = "SGP30-LIB";
...
void sgp30_init(sgp30_dev_t *sensor, sgp30_read_fptr_t user_i2c_read, sgp30_write_fptr_t user_i2c_write) {
    ...
    ESP_LOGI(TAG_2, "%s - Serial Number: %02x %02x %02x", __FUNCTION__, sensor->serial_number[0],
                                sensor->serial_number[1], sensor->serial_number[2]);

    uint16_t featureset;
    sgp30_execute_command(sensor, GET_FEATURE_SET_VERSION, 2, 10, &featureset, 1);
    ESP_LOGI(TAG_2, "%s - Feature set version: %04x", __FUNCTION__, featureset);
...
void sgp30_set_humidity(sgp30_dev_t *sensor, uint32_t absolute_humidity) {
    if (absolute_humidity > 256000) {
        ESP_LOGW(TAG_2, "%s - Abs humidity value %d is too high!", __FUNCTION__, absolute_humidity);
        return;
    }
...
static esp_err_t sgp30_execute_command(sgp30_dev_t *device, uint8_t command[], uint8_t command_len, uint16_t delay, 
    ...
    **********************************************************************************************/
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG_2, "Failed to write SGP30 I2C command! err: 0x%02x", err);
        return err;  
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG_2, "Failed to read SGP30 I2C command reply! err: 0x%02x", err);
    ...
    for (uint8_t i = 0; i < read_len; i++) {
        ...
        ESP_LOGD(TAG_2, "%s - Calc CRC: %02x,   Reply CRC: %02x", __FUNCTION__, crc, reply_buffer[i * 3 + 2]);

        if (crc != reply_buffer[i * 3 + 2]) {
            ESP_LOGW(TAG_2, "Reply and Calculated CRCs are different");
            return false;
        }
        ...
        ESP_LOGD(TAG_2, "%s - Read data: %04x", __FUNCTION__, read_data[i]);
    }
